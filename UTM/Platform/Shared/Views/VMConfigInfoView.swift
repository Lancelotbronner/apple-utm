...
struct VMConfigInfoView: View {
        ...
        case .operatingSystem:
            ...
            VStack {
                ...
                Button(action: { imageSelectVisible.toggle() }, label: {
                    ...
                    IconSelect(current: config.icon, onIconSelected: imageSelected)
                }
            }
            .frame(width: 90)
            #else
    ...
    private func imageSelected(icon: UTMIcon) {
        config.icon = icon
        imageSelectVisible = false
    }
}
...
private struct IconSelect: View {
    let current: UTMIcon?
    let onIconSelected: (UTMIcon) -> Void
    private let gridLayout = [GridItem(.adaptive(minimum: iconGridSize))]

    private static let icons: [String] = [
        "AIX", "IOS", "almalinux", "alpine", "amigaos", "android", "appleTv", "archLinux",
        "backtrack", "bada", "beos",
        "centos", "chromeOs", "cyanogenmod",
        "debian",
        "elementaryOs",
        "fedora", "firefoxOs", "freebsd",
        "gentoo",
        "haikuOs", "hpUx",
        "kaios", "knoppix", "kubuntu",
        "linux", "lubuntu",
        "mac", "mint", "maemo", "meego", "mandriva",
        "nixos", "netbsd", "nintendo",
        "os2", "openbsd", "openwrt",
        "palmos", "playstation", "popOs", "playstationPortable",
        "redHat", "riscOs", "remixOs",
        "suse", "sabayon", "solaris", "symbian", "syllable", "slackware", "sailfishOs",
        "tizen", "threadx",
        "ubuntu",
        "webos", "windows", "windows7", "windows11", "windows9X", "windowsXp",
        "xbox", "xubuntu",
        "yunos",
    ]
    ...
    var body: some View {
        LazyVGrid(columns: gridLayout, spacing: 0) {
            ForEach(IconSelect.icons, id: \.self) { icon in
                Button(action: { onIconSelected(.builtin(icon)) }, label: {
                    VStack(alignment: .center) {
                        Logo(logo: Image(icon))
                        Text(iconToTitle(.builtin(icon)))
                    ...
                    .overlay(
                        RoundedRectangle(cornerRadius: 10)
                            .stroke(current == .builtin(icon) ? Color.accentColor : Color.clear, lineWidth: 2)
                    )
                }).buttonStyle(.plain)
            }
...
private func iconToTitle(_ icon: UTMIcon) -> LocalizedStringKey {
    switch icon {
    case let .builtin(name): ICON_TITLE_MAP[name] ?? "Custom"
    case .custom: "Custom"
    default: "None"
    }
}
