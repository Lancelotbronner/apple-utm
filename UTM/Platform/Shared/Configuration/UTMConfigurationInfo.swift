...
enum UTMIcon: Hashable {
    case builtin(_ name: String)
...
struct UTMConfigurationInfo: Codable {
    ...
    var icon: UTMIcon {
        get {
            guard let iconURL else { return .none }
            return isIconCustom ? UTMIcon.custom(iconURL) : .builtin(iconURL.deletingPathExtension().lastPathComponent)
        }
        set {
            switch newValue {
            case let .builtin(name):
                isIconCustom = false
                iconURL = URL(fileURLWithPath: name)
            case let .custom(url):
                isIconCustom = true
                iconURL = url
            case .none:
